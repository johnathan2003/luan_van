...
class Shop(Base):
    ...
    verification_status = Column(Enum("pending", "approved", "rejected", native_enum=False), default="pending", index=True)
    verified_at = Column(DateTime)
    # Mall tier: shop được xét duyệt lên BuyZo Mall
    is_mall          = Column(Boolean, default=False, nullable=False)
    mall_request_status = Column(Enum("none", "pending", "approved", "rejected", native_enum=False), default="none")
...
class ShopEmployee(Base):
    ...
    status = Column(Enum("active", "inactive", "suspended", native_enum=False), default="active", index=True)
...
class SystemEmployeePermission(Base):
    ...
    scope = Column(Enum("admin", "shop", "both", native_enum=False), default="admin")
    granted_by = Column(Integer, ForeignKey("users.user_id"), nullable=False)
    granted_at = Column(DateTime, server_default=func.now())
