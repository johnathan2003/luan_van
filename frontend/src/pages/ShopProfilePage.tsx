...
const IMG_SPEC: Record<ImageType, { w: number; h: number; label: string; ratio: string; maxKB: number }> = {
  cover: { w: 1200, h: 300, label: 'Ảnh bìa',  ratio: '4:1',  maxKB: 2048 },
        ...
        <div style={{ padding: 24 }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
              {[
                { icon: '↔️', label: 'Kích thước khuyến nghị', val: `${spec.w} × ${spec.h} px` },
                { icon: '📐', label: 'Tỉ lệ khung hình',        val: spec.ratio },
                { icon: '💾', label: 'Dung lượng tối đa',       val: `${spec.maxKB >= 1024 ? spec.maxKB / 1024 + ' MB' : spec.maxKB + ' KB'}` },
              ].map(r => (
            ...
            <p style={{ fontSize: 11, color: C.gray, margin: '10px 0 0' }}>
              💡 Ảnh có thể lớn hơn nhưng phải giữ tỉ lệ {spec.ratio}. Định dạng: JPG, PNG, WEBP.
            </p>
          </div>
            ...
            {preview ? (
              ...
              <>
                ...
                <p style={{ fontSize: 12, color: C.gray, margin: 0 }}>
                  Hỗ trợ JPG, PNG, WEBP — tối đa {spec.maxKB >= 1024 ? spec.maxKB / 1024 + 'MB' : spec.maxKB + 'KB'}
                </p>
              </>
            )}
          </div>
          ...
          {val && !checking && (
            <div style={{
              ...
              <p style={{ fontWeight: 700, color: val.ok ? C.success : C.error, margin: '0 0 10px', fontSize: 13 }}>
                {val.ok ? '✅ Ảnh hợp lệ — sẵn sàng sử dụng!' : '⚠️ Ảnh chưa đạt yêu cầu — xem chi tiết bên dưới'}
              </p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                {[
                  ...
                  <div key={row.label} style={{
                    ...
                    <p style={{ fontSize: 10, color: C.gray, margin: 0 }}>Yêu cầu: {row.req}</p>
                  </div>
                ))}
              </div>
              {!val.ok && (
                <p style={{ fontSize: 11, color: C.error, margin: '10px 0 0' }}>
                  💡 Gợi ý: {!val.dimOk ? `Vui lòng chọn ảnh có tỉ lệ gần ${spec.ratio} và kích thước tối thiểu ${Math.round(spec.w * 0.5)}×${Math.round(spec.h * 0.5)}px.` : ''} {!val.sizeOk ? `Ảnh quá nặng, vui lòng nén ảnh dưới ${spec.maxKB >= 1024 ? spec.maxKB / 1024 + 'MB' : spec.maxKB + 'KB'}.` : ''}
                </p>
              )}
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 20, justifyContent: 'flex-end' }}>
            ...
            >
              {file && val && !val.ok ? '⚠️ Dung luu du biet loi' : '✓ Xác nhận & Lưu'}
            </button>
          </div>
...
const ShopProfilePage: React.FC = () => {
  ...
  // tich tien do nhiem vu "xem trang cua 1 shop" cho sự kiện
  useEffect(() => {
  ...
  if (!shop) return (
    <div style={{ textAlign: 'center', padding: 80 }}>
      ...
      <p style={{ color: C.gray, marginBottom: 16 }}>Shop không tồn tại hoặc đã đóng cửa</p>
      <Link to="/products" className="btn btn-primary">Quay lại mua sắm</Link>
    </div>
  )
      ...
      <div className="container" style={{ paddingTop: 28, paddingBottom: 60 }}>
        ...
        {tab === 0 && (
          <div>
            ...
            {/* Het sản phẩm cua shop -> goi y them sản phẩm khac de người dùng tiep tuc xem */}
          ...
          <div style={{ maxWidth: 680 }}>
            <div style={{ background: 'white', borderRadius: 16, padding: 28, border: `1px solid ${C.border}`, marginBottom: 24, display: 'flex', gap: 40, alignItems: 'center' }}>
              <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ fontSize: 13, color: C.gray }}>Danh giá trung binh</p>
              </div>
              <div style={{ flex: 1 }}>
                {[5, 4, 3, 2, 1].map(star => {
            ...
            <p style={{ color: C.gray, textAlign: 'center', fontSize: 14 }}>Tính năng đánh giá đang được phát triển.</p>
          </div>
        )}
          ...
          <div style={{ maxWidth: 600 }}>
            <div style={{ background: 'white', borderRadius: 16, padding: 28, border: `1px solid ${C.border}`, marginBottom: 20 }}>
              ...
              {[
                ...
                { icon: '&#128197;', label: 'Tham giá',    value: shop.created_at ? new Date(shop.created_at).toLocaleDateString('vi-VN') : 'N/A' },
              ].map(row => (
