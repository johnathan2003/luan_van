...
const DEFAULT_RATES: Rates = { shop_rate: 70, admin_rate: 15, shipper_rate: 5, vat_rate: 10 }
...
const ShopRevenueDetailPage: React.FC = () => {
  ...
  // Fetch dynamic revenue rates once
  useEffect(() => {
    API.get('/api/v1/system/revenue-config')
      .then((r: any) => {
        const d = r.data
        if (d?.shop_rate != null) {
          setRates({
            shop_rate:    Number(d.shop_rate),
            admin_rate:   Number(d.admin_rate),
            shipper_rate: Number(d.shipper_rate),
        ...
        if (analytics) {
          ...
          if (analytics.revenue_by_month && Array.isArray(analytics.revenue_by_month)) {
            ...
            for (let i = 5; i >= 0; i--) {
              ...
              months.push({ month: monthLabel(month), income: Math.random() * 10000000, orders: Math.floor(Math.random() * 20) })
            }
            setMonthly(months)
          }
        }
  ...
  const shopPct    = rates.shop_rate / 100
  const adminPct   = rates.admin_rate / 100
  const shipperPct = rates.shipper_rate / 100
  const vatPct     = rates.vat_rate / 100
  const feePct     = 1 - shopPct   // = (admin + shipper + vat) / 100

  const profit    = (balance?.total_revenue ?? 0) * shopPct
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {/* Thông tin chi phí hoa hồng — dynamic */}
      <div style={{ background: '#F0FDF4', border: '1px solid #BBEF63', borderRadius: 8, padding: '12px 16px', marginBottom: 16, fontSize: 12, color: C.navy }}>
        <p style={{ margin: '0 0 6px', fontWeight: 600 }}>📋 Cách tính lợi nhuận (theo cấu hình hiện tại):</p>
        <p style={{ margin: '0 0 4px' }}>• Doanh thu brutto: 100% (tiền khách thanh toán)</p>
        <p style={{ margin: '0 0 4px' }}>• Chi phí sàn: -{Math.round(feePct * 100)}%</p>
        <p style={{ margin: '0 0 4px', paddingLeft: 16 }}>
          └─ Admin: {rates.admin_rate}% | Shipper: {rates.shipper_rate}% | VAT: {rates.vat_rate}%
        </p>
        <p style={{ margin: 0, fontWeight: 600 }}>• Lợi nhuận shop: {rates.shop_rate}% (số tiền thực nhận)</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {kpis.map(s => (
          <div key={s.label} style={{
            ...
            <p style={{ fontSize: 11, fontWeight: 600, color: s.big ? 'rgba(255,255,255,0.8)' : C.gray, textTransform: 'uppercase', marginBottom: 6 }}>
              {s.label}
            </p>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: C.tint }}>
                {[
                  'Ngày', 'Đơn hàng', 'Loại', 'Ghi chú',
                  'Doanh thu',
                  `Chi phí sàn (-${Math.round(feePct * 100)}%)`,
                  `Lợi nhuận (${rates.shop_rate}%)`,
                  'Trạng thái',
                ].map(h => (
                ...
                return (
                  <tr key={t.txn_id} style={{ borderBottom: `1px solid ${C.tint}` }}
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 9px', borderRadius: 20, background: meta.bg, color: meta.color }}>
                        {meta.label}
                      </span>
                    </td>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      {t.type === 'sale' && t.amount > 0 ? (() => {
                        ...
                        const admin   = Math.round(gross * adminPct)
                        const shipper = Math.round(gross * shipperPct)
                        const vat     = Math.round(gross * vatPct)
                        return (
                          <div style={{ lineHeight: 1.6 }}>
                            <div style={{ fontWeight: 700, fontSize: 13, color: C.error }}>-{fmt(fee)}</div>
                            <div style={{ fontSize: 10, color: C.gray }}>
                              <span title={`Admin ${rates.admin_rate}%`}>🏢 {fmt(admin)}</span>
                              {' · '}
                              <span title={`Shipper ${rates.shipper_rate}%`}>🚚 {fmt(shipper)}</span>
                              {' · '}
                              <span title={`VAT ${rates.vat_rate}%`}>🧾 {fmt(vat)}</span>
                            </div>
                          </div>
                        )
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      {t.type === 'sale' && t.amount > 0 ? (() => {
                        const p = Math.round(t.amount * shopPct)
                        return (
                          <div>
                            <div style={{ fontWeight: 800, fontSize: 14, color: C.green }}>+{fmt(p)}</div>
                            <div style={{ fontSize: 10, color: C.gray }}>{rates.shop_rate}% doanh thu</div>
                          </div>
                        )
                      })() : <span style={{ color: C.gray }}>—</span>}
                    </td>
