...
const VoucherManagementPage: React.FC = () => {
  ...
  const [form, setForm] = useState({ code: '', discount_type: 'percentage', discount_value: 10, min_order_value: '', max_uses: '', max_discount: '' })
  ...
  const handleCreate = async () => {
    try {
      await shopService.createVoucher({
        ...form,
  ...
  return (
    <div>
      ...
      <div className="card table-wrapper">
        <table>
          <thead><tr><th>Mã</th><th>Loại</th><th>Giá trị</th><th>Giảm tối đa</th><th>Đã dùng</th><th>Tối đa</th><th>Trạng thái</th></tr></thead>
          <tbody>
            {vouchers.map(v => (
              <tr key={v.voucher_id}>
                ...
                <td style={{ color: 'var(--gray-500)' }}>{v.max_discount ? `${Number(v.max_discount).toLocaleString('vi-VN')}₫` : '—'}</td>
