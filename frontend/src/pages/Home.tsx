...
import { getFlashSaleActiveShops, getFlashSaleDisplayInfo } from '../utils/flashSalePoolStore'
import { getFlashProductList, getTopProductListByShop } from '../utils/bannerDraftStore'
...
const FlashSaleSection: React.FC = () => {
  ...
  useEffect(() => {
    const flashShops: Set<string> = getFlashSaleActiveShops()
    // Tính thời gian còn lại (lấy max của các shop đang hoạt động)
    let maxRemaining = 0
    flashShops.forEach(shopName => {
      const info = getFlashSaleDisplayInfo(shopName)
      if (info.remainingMs > maxRemaining) maxRemaining = info.remainingMs
    })
    setRemainingMs(maxRemaining)

    // Lấy sản phẩm đã tích chọn Flash Sale từ localStorage của các shop thanh toán đủ
    const localItems: any[] = []
    flashShops.forEach(shopName => {
      const products = getFlashProductList(shopName)
      products.forEach(p => {
        if (p && p.selectedForFlashSale) localItems.push({
          product_id: p.productId ?? null,
          product_name: p.productName,
          price: p.price,
          image_urls: [p.productImage],
          rating: null,
          sales_count: null,
          _fromFlashPool: true,
          shop_name: shopName,
    ...
    for (let i = localItems.length - 1; i > 0; i--) {
      ...
      [localItems[i], localItems[j]] = [localItems[j], localItems[i]]
    }
