 ...
 * CRUD + gửi broadcast đến các nhóm người dùng
 */
...
const SystemNotificationPage: React.FC = () => {
  ...
  const load = () => {
    setLoading(true)
    API.get('/api/v1/admin/system-notifications', { params: { limit: 50 } })
      .then((r: any) => setNotiList(r.data?.notifications ?? []))
      .catch(() => setNotiList([]))
  ...
  const openEdit = (n: any) => {
    setForm({ title: n.title, content: n.content, type: n.type, audience: n.audience, send_at: n.send_at?.slice(0, 16) ?? '' })
    setEditId(n.id)
    setShowForm(true)
  }
  ...
  const remove = async (id: number) => {
    ...
    try {
      ...
      setNotiList(ns => ns.filter(n => n.id !== id))
      toast.success('Đã xóa thông báo')
    } catch (err: any) {
      toast.error(err.response?.data?.detail || 'Lỗi xóa thông báo')
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editId !== null) {
        await API.put(`/api/v1/admin/system-notifications/${editId}`, payload)
        toast.success('Đã cập nhật thông báo')
      } else {
        const r: any = await API.post('/api/v1/admin/system-notifications', payload)
        toast.success(r.data?.sent ? 'Đã tạo và gửi thông báo ngay' : 'Đã tạo thông báo (sẽ gửi theo lịch)')
      }
      setShowForm(false)
      load()
    } catch (err: any) {
      toast.error(err.response?.data?.detail || 'Lỗi lưu thông báo')
    } finally {
      setSaving(false)
    }
  }
  ...
  const sendNow = async (id: number) => {
    try {
      await API.post(`/api/v1/admin/system-notifications/${id}/send`)
      toast.success('Đã gửi thông báo đến người dùng')
      setNotiList(ns => ns.map(n => n.id === id ? { ...n, sent: true } : n))
    } catch (err: any) {
      toast.error(err.response?.data?.detail || 'Lỗi gửi thông báo')
    }
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {/* List */}
      {loading ? (
        <div style={{ textAlign: 'center', padding: 48, color: C.gray }}>Đang tải...</div>
      ) : notiList.length === 0 ? (
        <div className="card" style={{ padding: 48, textAlign: 'center', color: C.gray }}>
          <p style={{ fontSize: 32, marginBottom: 12 }}>📣</p>
          <p style={{ fontSize: 15, fontWeight: 600 }}>Chưa có thông báo hệ thống nào</p>
          <p style={{ fontSize: 13, marginTop: 4 }}>Nhấn "+ Thêm thông báo" để tạo thông báo đầu tiên</p>
        </div>
      ) : (
              ...
              <div key={n.id} className="card" style={{ padding: '16px 20px', display: 'flex', gap: 16, alignItems: 'flex-start', borderLeft: `4px solid ${tp.color}` }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    ...
                    {n.send_at && <span style={{ fontSize: 11, color: C.warning }}>⏰ {n.send_at?.slice(0, 16)}</span>}
                    <span style={{ fontSize: 11, color: C.gray }}>🕐 {n.created_at?.slice(0, 16)}</span>
                    <span style={{ fontSize: 11, fontWeight: 700, padding: '1px 8px', borderRadius: 10, background: n.sent ? '#DCFCE7' : '#FEF3C7', color: n.sent ? C.success : C.warning }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
                  {!n.sent && (
                    <button onClick={() => sendNow(n.id)}
                      style={{ padding: '6px 14px', background: '#DCFCE7', color: C.success, border: 'none', borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                      🚀 Gửi ngay
                    </button>
                  )}
                  <button onClick={() => openEdit(n)}
                    style={{ padding: '6px 14px', background: C.light, color: C.blue, border: 'none', borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                    ✏️ Sửa
                  </button>
                  <button onClick={() => remove(n.id)}
                    style={{ padding: '6px 14px', background: '#FEE2E2', color: C.error, border: 'none', borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                    🗑️ Xóa
                  </button>
                </div>
              </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div className="card" style={{ width: 500, padding: 28, maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: C.gray, display: 'block', marginBottom: 4 }}>Hẹn giờ gửi (để trống = gửi ngay)</label>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button onClick={handleSave} disabled={saving} style={{ flex: 2, padding: '10px', background: C.blue, color: 'white', border: 'none', borderRadius: 9, fontWeight: 700, cursor: 'pointer' }}>
                {saving ? '⏳ Đang lưu...' : editId ? 'Lưu thay đổi' : (form.send_at ? 'Hẹn gửi' : 'Tạo & Gửi ngay')}
              </button>
            </div>
          </div>
        </div>
